...
%start body
%type <context * context * Syntax.term > body
%%
...
(* values *)
  Val
  { $1 }
(* discrete tensor product elimination *)
| DLET LPAREN ID COMMA ID RPAREN EQUAL Term SEMI Term
  { fun ctx dctx ->
    let dctx_x  = extend_dummy_var $3.v dctx   in
    let dctx_xy = extend_dummy_var $5.v dctx_x in
...
| LET LPAREN ID COMMA ID RPAREN EQUAL Term SEMI Term
  { fun ctx dctx ->
    let ctx_x  = extend_dummy_var $3.v ctx   in
    let ctx_xy = extend_dummy_var $5.v ctx_x in
...
| UNIONCASE Val OF LBRACE INL LPAREN ID RPAREN DBLARROW Term PIPE 
  ...
  { fun ctx dctx ->
    let ctx_l = extend_dummy_var $7.v  ctx in
    let ctx_r = extend_dummy_var $14.v ctx in
...
TyArgument :
  ...
  { fun ctx -> (extend_var $2.v ($4 ctx) ctx) }
